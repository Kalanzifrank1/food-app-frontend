...
export const useGetMyUser = () => {
  ...
  const { data: currentUser, isPending, error } = useQuery({
    queryKey: ['fetchCurrentUser'], // Query key should be an array
    queryFn: getMyUserRequest // Your fetch function
  });
...
export const useCreateMyUser = () => {
  ...
  const { mutateAsync: createUser, isPending, isError, isSuccess } = 
    useMutation({ mutationFn: createMyUserRequest }); // <-- Note: `mutationFn` in v5
...
export const useUpdateMyUser = () => {
  ...
  return {
    updateUser, isPending, isSuccess, error, reset
  }
